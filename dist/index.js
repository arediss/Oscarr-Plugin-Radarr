...
var RadarrPluginApi = class {
  ...
  // ── Releases ───────────────────────────────────────────────────────
  async getReleasesForMovie(movieId) {
  ...
  async getQueueForMovie(movieId) {
    const { data } = await this.api.get("/queue", {
  ...
  async getBlocklistForMovie(movieId) {
...
var CACHE_TTL = 6e4;
async function getCachedMovies(api) {
  const now = Date.now();
...
function analyticsRoutes(app) {
  app.get("/analytics", async (request) => {
    ...
    return {
      ...
      qualityDistribution: Object.entries(qualityDist).map(([name, count]) => ({ name, count })).sort((a, b) => b.count - a.count),
...
async function getApi(ctx) {
  if (radarrApi) return radarrApi;
  const config = await ctx.getServiceConfig("radarr");
  if (!config) throw new Error("Radarr service not configured in Oscarr");
  radarrApi = new RadarrPluginApi(config.url, config.apiKey);
  return radarrApi;
}
  ...
  return {
    ...
    async registerRoutes(app) {
      app.addHook("preHandler", async (request, reply) => {
        try {
          request.radarrApi = await getApi(ctx);
        } catch {
          return reply.status(503).send({ error: "Radarr not configured or unreachable" });
        }
      });
      libraryRoutes(app);
      qualityRoutes(app);
      ...
      filesRoutes(app);
      downloadsRoutes(app);
      ctx.log.info("Radarr Manager plugin routes registered");
    }
  };
}
export {
